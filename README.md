...
# Set a password (or use SSH key later).

Step 2 — Add git-wfms to ftp_wfms group
sudo usermod -aG ftp_wfms git-wfms

Step 3 — Set proper folder permissions
...
#!/bin/bash
# Restricted shell for git-health user
...
/usr/local/bin/restricted-wfms.sh
...
Run any option (e.g., 3 → composer update or 8 → php artisan optimize:clear).
...
bottom of the file, and paste it there.
Match User git-wfms
    ForceCommand /usr/local/bin/restricted-wfms.sh
    AllowTcpForwarding no
    X11Forwarding no

# IMPORTANT RULE The Match block must always be the last section in the file.
...
Now you can give the ssh permission with port:2223 through the firewall.
